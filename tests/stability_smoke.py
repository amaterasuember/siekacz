...
def test_ui_validation_loader_and_export() -> None:
    ...
    window.calculate()
    app.processEvents()
    assert window._is_calculating
    assert window.optimization_progress_overlay.isVisible()
    _process_events(3400)
    app.processEvents()
    assert not window._is_calculating
    assert not window.optimization_progress_overlay.isVisible()
    assert window.last_result is not None
    assert "Gilotynowo" not in window.statusBar().currentMessage()
    assert_layout_invariants(window._project_for_calculation(window._collect_parts()), window.last_result, expect_all_placed=True)
